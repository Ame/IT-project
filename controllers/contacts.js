...
module.exports = class ContactsController {
  static async apiAddContact(req, res) {
    ...
    Contact.findOne({ email: req.body.email, id: req.body.id }).then(
      (contact) => {
  ...
  static async apiAddTag(req, res, next) {
    Contact.findOne({ email: req.body.email, user: req.user.id }).then(
      (contact) => {
        if (!contact) {
        ...
        // now update the tag
        Contact.updateOne(
          { email: req.body.email, user: req.user.id },
          { tags: newTags }
        ).then((result) => {
